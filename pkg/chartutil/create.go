...
{{- else if contains "ClusterIP" .Values.service.type }}
  export POD_NAME=$(kubectl get pods --namespace {{ .Release.Namespace }} -l "app.kubernetes.io/name={{ include "<CHARTNAME>.name" . }},app.kubernetes.io/release={{ .Release.Name }}" -o jsonpath="{.items[0].metadata.name}")
...
app.kubernetes.io/cluster: {{ .Release.DemeterCluster }}
app.kubernetes.io/appsuite: {{ .Release.DemeterAppSuite }}
{{- end }}
